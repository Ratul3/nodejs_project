...
module.exports ={
	...
	get: function(id, callback){
		var sql = "select * from pnotices where id="+id;
...
getByUname: function(username, callback){
	...
	insert: function(user, callback){
		var sql = "insert into pnotices values('"+user.id+"', '"+user.uname+"', '"+user.notice+"', '"+user.date+"','')";
...
var sql = "update salary set name='"+user.name+"', salary='"+user.salary+"', teacher_name='' where id='"+user.id+"'";
	...
	delete: function(id, callback){
		var sql = "delete from pnotices where id="+id;
		db.execute(sql, function(status){
			if(status){
				callback(true);
			}else{
